...
**Boiler Upgrade Scheme (BUS) grant terms and cost data**: Ofgem BUS scheme page; Nesta, "Four years of the Boiler Upgrade Scheme in four charts" (30 April 2026, using DESNZ scheme microdata).
[Ofgem BUS scheme](https://www.ofgem.gov.uk/environmental-and-social-schemes/boiler-upgrade-scheme-bus) · [Nesta analysis](https://www.nesta.org.uk/blog/four-years-boiler-upgrade-scheme-four-charts/)
Used for: `check_combined_capex_hypothesis.py` — the flat £7,500 BUS grant per ASHP installation (£9,000 for oil/LPG households from 21 July 2026), regardless of unit size; median UK ASHP installed cost £13,041 (all sizes blended, DESNZ microdata via Nesta); confirmation that most installations now cluster in the 4–12kW range rather than 12kW+.
Status: **GROUNDED** for the flat grant amount and the median all-sizes cost (primary-adjacent — Nesta's analysis is built directly on DESNZ scheme data, not survey-estimated). **PROVISIONAL** for the specific size-banded cost figures used in the combined-CapEx check below — DESNZ/Nesta's own cost-by-size breakdown exists only as a rendered chart image, not machine-readable data, so the £10,500 (6-8kW) and £7,500 (4-6kW) figures used are triangulated from multiple 2026 UK installer/aggregator surveys instead, a materially weaker evidentiary tier than the rest of this citation.

**UK domestic battery storage installed cost by discrete capacity tier, multiple 2026 UK market surveys** (installer/aggregator sites, same evidentiary tier as the existing flat-£/kWh battery citation above).
Used for: `check_combined_capex_hypothesis.py` — 5kWh-class systems ~£2,500–4,000 installed, 10kWh-class ~£4,000–7,000, 13.5kWh premium (Tesla Powerwall 3) ~£8,500–10,500; midpoints (£3,250 / £5,500) used for the combined-CapEx check.
Status: **PROVISIONAL** — vendor/installer-survey tier, consistent with the standard already applied to this project's existing flat-£/kWh battery-cost citation.

**Warm Homes: Social Housing Fund Wave 3, cost caps and eligibility**: Menzies LLP (specialist advisory firm working on WHSHF Wave 3 audit/assurance compliance), 24 February 2026; cross-checked against City Science and Zero Carbon Accelerator summaries of the same scheme.
[Menzies LLP, "Warm Homes: Social Housing Fund Wave 3"](https://www.menzies.co.uk/warm-homes-social-housing-fund-wave-3/)
Used for: `check_combined_capex_hypothesis.py` and the "sequencing" point in `linkedin_article_business_case.md`/`STAKEHOLDER_BRIEFS.md` — real per-property cost caps (£7,500/home base, any measure, 50:50 match funded, any starting EPC band; +£7,500 for off-gas-grid low-carbon heating; up to £20,000/home for a 10%-of-application quota on-gas-grid); core scope is EPC bands D–G, with EPC-C+ homes eligible only via the 10%-quota/infill route.
Status: **GROUNDED** — primary-adjacent (a specialist advisory firm's direct summary of scheme terms for grant-recipient clients, not a lead-generation aggregator), corroborated by two independent secondary summaries. Supersedes the earlier, vaguer £1.29bn-total-programme figure as this project's grounding for WHSHF's actual per-property mechanics.
